...
const ECOLEDIRECTE_API_VERSION =
  process.env.ECOLEDIRECTE_API_VERSION || "4.87.0";

if (!IDENT || !PASS) {
  console.error(
    "⚠️  Renseigne ECOLEDIRECTE_IDENTIFIANT et ECOLEDIRECTE_MDP dans .env"
  );
  process.exit(1);
}
...
function parseCookies(rawHeader) {
  if (!rawHeader) return {};
  const str = Array.isArray(rawHeader)
    ? rawHeader.join("; ")
    : String(rawHeader);
  const cookies = {};
...
async function fetchCookies() {
  ...
  const raw =
    res.headers?.raw?.()["set-cookie"] || res.headers.get("set-cookie") || "";
  ...
  if (String(longValue).length !== 462)
    console.warn(
      `⚠️ Cookie 462-char introuvable — fallback ${longName} (len=${longValue.length})`
    );
...
(async () => {
  try {
    ...
    if (resp?.code === 200 && resp.token) {
      ...
      console.error(
        `❌ Login échoué (${resp.code}) : ${
          resp.message || resp.error || "Erreur inconnue"
        }`
      );
    }
  } catch (err) {
