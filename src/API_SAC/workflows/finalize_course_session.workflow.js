// ./API_SAC/workflows/finalize_course_session.workflow.js
const { DateTime } = require("luxon");
const { prisma } = require("../commons/prisma.common");
const { sendMail } = require("../commons/mail.common");
const { LOG_DESTINATIONS, TECHNICAL_LEVELS, log_business, log_technical } = require("../commons/logger.common");
const { get_data_by_type } = require("../../scripts/auto/get_data.script");
const {
  generate_attendance_pdf,
  generate_daily_attendance_pdf,
} = require("../../scripts/auto/generate_attendance_pdf.script");
...
async function is_session_finalized(sessionId) {
...
async function sendSessionPdfToTeacher({ session, pdfBuffer, filename }) {
  ...
  if (!teacherEmail) {
    log_technical(TECHNICAL_LEVELS.WARNING, "Session PDF email skipped: missing teacher email", {
      sessionId: session.id,
...
async function sendDailyPdfToPersonnelIfLastSession({ session, author }) {
  if (!(await isLastSessionOfClassDay(session))) {
    log_technical(TECHNICAL_LEVELS.VERBOSE, "Daily PDF skipped: session is not last class session of day", {
      sessionId: session.id,
      classId: session.classId,
    });
    return {
  ...
  if (!recipients.length) {
    log_technical(TECHNICAL_LEVELS.WARNING, "Daily PDF email skipped: no personnel recipients", {
      sessionId: session.id,
      classId: session.classId,
    });
    return {
  ...
  const day = toParisDateTime(session.startTime).toISODate();
  const { pdfBuffer } = await generate_class_day_pdf({
    classId: session.classId,
    date: day,
    author,
  });
...
async function prepare_finalize_from_nfc(req) {
  ...
  if (await is_session_finalized(session.id)) {
...
async function finalize_session(req) {
  const user = getSessionUser(req);
  if (!user) return response(403, "TEACHER_REQUIRED", "Seul l'enseignant peut valider l'appel.");
  ...
  const edResult = await get_data_by_type("APPEL", {
  ...
  const pdfBuffer = await generate_attendance_pdf(sessionToPdfDataFromStudents(session, dbStudents, author));
  ...
  await log_business("course_session_finalized", "Appel envoyé à EcoleDirecte et PDF généré.", {
    req,
    ...
    metadata: {
      ...
      horaire,
      presentCount: eleves.filter(eleve => !eleve.isAbsent).length,
      absentCount: eleves.filter(eleve => eleve.isAbsent).length,
      pdfFilename: filename,
      sessionPdfSentTo,
      dailyPdf,
    },
...
async function generate_class_day_pdf({ classId, date, author = "SAC" }) {
  ...
  const pdfBuffer = await generate_daily_attendance_pdf(data);
